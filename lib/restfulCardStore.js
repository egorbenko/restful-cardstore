...
class RestfulCardStore extends BusinessNetworkCardStore {
  ...
  /**
   * Constructor.
   * @param {string} baseUrl Your RESTful API url
   * @param {Object} options Additional configuration options for the card store.
   * @param {function} callback
   */
  constructor(baseUrl, options, callback) {
    if (!baseUrl) {
      throw new Error(`RestfulCardStore requires baseUrl to be passed into constructor.`)
    }
    super();
  ...
  async put (cardName, card) {
    try {
      return this.callback(await this.xhr.put(`${this.setting.putPath}/${cardName}`, { idCard: JSON.stringify(card) }));
    } catch (err) {
