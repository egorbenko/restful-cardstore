...
class RestfulCardStore extends BusinessNetworkCardStore {
  ...
  async get (cardName) {
    try {
      let { metadata, connectionProfile, credentials } = await this.callback(await this.xhr.get(`${this.setting.getPath}/${cardName}`));
  ...
  async put (cardName, card) {
    try {
      return this.callback(await this.xhr.put(`${this.setting.putPath}/${cardName}`, qs.stringify({ card: JSON.stringify(card) })));
    } catch (err) {
  ...
  async getAll () {
    try {
      return this.callback(await this.xhr.get(this.setting.getPath));
    } catch (err) {
