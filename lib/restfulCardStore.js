...
class RestfulCardStore extends BusinessNetworkCardStore {
  ...
  async get (cardName) {
    try {
      return this.callback(await this.xhr.get(`${this.setting.getPath}/${cardName}`));
    } catch (err) {
  ...
  async put (cardName, card) {
    try {
      return this.callback(await this.xhr.put(`${this.setting.putPath}/${cardName}`, qs.stringify(card)));
    } catch (err) {
